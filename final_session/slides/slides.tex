...
\begin{frame}{Setup}
  \begin{itemize}
    \item Cora node classification, DeepRobust \texttt{nettack} split (2485 nodes), CPU.
...
\begin{frame}{Findings}
  \begin{itemize}
    \item Adding (FoSR) \& spectral removal (ProxyDelete): \textbf{no robustness gain} --- track the GCN.
      (FoSR even triples the spectral gap, its own objective --- still no gain.)
    \item Only \textbf{attack-aware defenses} defend: Jaccard (similarity) \& SVD (low-rank).
    \item[$\Rightarrow$] The editing \emph{criterion} matters, not the add/remove \emph{direction}.
  \end{itemize}
  \vfill
  \footnotesize An honest negative result for the over-squashing $\to$ robustness bridge --- real numbers only.
\end{frame}
